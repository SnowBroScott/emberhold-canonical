...
DATE: 2026-08-03
WHY: The crux is that in a shared-session model no server-side check can ever distinguish a Kin from a Keeper, because the only evidence of who is acting is client-supplied. auth.uid() is always the owner; useActiveMember() is a localStorage key; there is no third source of truth and one cannot be built from the two that exist. Every attempt to close the audience reads inside this model ends in the same circle: the RPC validates that the passed profile belongs to the household, and a Kin with devtools passes a Keeper's profile id. So the four waiting items are not four fixes awaiting a decision. They are four things own-session can only accept and per-member auth can only close. Per-member auth was rejected on four grounds. First, kids have no email, so it requires building a credential path, and kid-auth was already DECLINED, making this a reopen of a settled call rather than a new one. Second, and this is the one that decided it, the wall does not benefit and cannot: the wall is one device any member walks up to, it can never hold a per-member session, so wall_request_redemption and the validated-actor-id pattern survive per-member auth unchanged. It does not replace the pattern, it adds a second authority model running alongside it. Third, per-member auth creates accounts for under-thirteens, which is the literal COPPA trigger, making a Phase 2 item that is already at least fifteen rows meaningfully more expensive; own-session collects nothing from Kin because Kin have no accounts. Fourth, switching becomes sign out and sign in on the shared family device, degrading the primary daily loop that Scott and May both use constantly. Scott's own framing: the only identity check anywhere after login is a PIN, and the PIN is now logging correctly, so the human boundary is already most of what per-member auth was intending. The precise residue: the PIN stops a Kin walking into a Keeper profile, which is the thing that actually happens in a house, and does not stop devtools, because the PIN gates the client and the database never sees it. Own-session accepts that the machine boundary is unenforceable and keeps the human one. THE SINGLE REOPEN TRIGGER IS FLAT OR PEER HOLDS. Roommates, a nanny, a non-custodial parent. The walk-up boundary is only sound because physical possession of the device equals household authority, which is true of a family and flatly false of roommates. Nothing at Gate B or Gate E forces the other branch; this fork was blocking estimates, not shipping.
REPLACES: The parked "own-session vs per-member-auth" open decision, critical path #1 since 2026-08-02. Also converts four items from conditional to settled: the adults_only rewards read and the parents_only quest details read are now permanently accepted and should be marked ignored on the Lovable panel with this reasoning attached; the "adult PIN lock isn't tied to real permission checks" and "any member can submit a redemption attributed to another member" findings lose their conditional footnote.
STATUS: LOCKED

---

DECISION: Parent and Kid are retired from the glass. The pair is Keeper and Kin.
DATE: 2026-08-03
WHY: The trigger was an identity question, and the answer to that question was no. Scott asked whether renaming "kid" could open Emberhold to roommates and shared houses. It cannot: the labels name an authority tier, not an age. Keeper means can mint; Kin means needs a second party. Rename both and the shape underneath is identical, and the real wall is that between peers there is no natural sink, so a peer economy has a mint and no bank. Chasing that group also reopens the fork, since flat holds break the walk-up boundary. So Emberhold stays a family management app, not a domicile management app, which is what the north-star already implies by claiming family-specific jobs as the edge. But the rename is still right for a different group: families the current labels already exclude. A sixteen-year-old is not a kid. A grandmother living in the hold is not a parent. Single-parent-plus-grandparent, an older sibling who genuinely approves, an au pair. Those are families, they are inside the thesis, and two nouns were keeping them out. Keeper won over Guardian because Guardian is the school-portal word, institutionally correct and completely without warmth, and it would read as imported from another app next to bounties and embers. Keeper is the same legibility with a verb behind it: a Keeper keeps the hold. It also survives being said to a nine-year-old at 7am, which Guardian barely does. Rejected along the way: Steward (the corporate-deck word), Warden (early favorite, killed on the prison read, though register does more work than a bare comparison suggests), Elder (warmest and most legible, killed because it smuggles age back in, which is the exact thing the rename exists to escape, and a sixteen-year-old Elder is as wrong as a sixteen-year-old Parent), Kindler (names the mechanic precisely and nobody on earth knows what it means, which is fatal for a label that gets no explanation), Keyholder, Reeve, Hearthkeeper, Anchor, Steady, Firstkin, Hearth. Kin locked first and independently as the strongest single noun generated. The objection that Keeper collides with the Keep register was raised and dismissed: registers gate nothing, anywhere, so nobody will infer a Garden hold cannot have Keepers. SCOPE IS GLASS ONLY. profiles.role keeps storing 'parent' and 'kid', exactly the way identifiers kept saying quest after Bounty won.
REPLACES: "Role-label retirement (Parent/Kid)", parked in LATER since before 2026-07-31.
STATUS: LOCKED

---

DECISION: The activity feed stores facts, never display text. actor_label splits into actor_id and subject_profile_id.
DATE: 2026-08-03
WHY: Recon proved what reasoning had only suspected. The column conflates two facts at four of eight write sites: vault.tsx:204, quest.$id.tsx:84, wall.tsx:166 and Briefing.tsx:185 all write the BENEFICIARY into actor_label while actor_id holds auth.uid(), which under own-session is always the owner. Sites 1, 4 and 8 write the actual actor. Same column, two meanings, no way to tell them apart from the row. So the column does two jobs because there was one column and two facts, and two facts get two columns: actor_id for who clicked, a validated subject_profile_id for who gets credit, verb stays an enum, names resolve at render time by joining to profiles. That is option (b) from the parking lot, now confirmed by evidence rather than by lean. On the blocking question of whether the display label belongs in the DB at all: no. The 08-01 finding that the renderers are not byte-identical is the confirmation rather than the puzzle. The wall omits "to the calendar" because on the wall the calendar is right there, which is a presentation decision belonging to the surface; storing the sentence would freeze one surface's phrasing into the database and take that latitude away permanently. A stored name also goes stale the moment a member changes their display name, showing history who they used to be. The one genuine argument for the other side, recorded so it stays rejected on purpose: a stored string survives a deleted member and an ID does not. The answer is soft-delete or rendering "a former Kin", and in a product whose tenant is a family, member churn is close to zero. A jAIne argument that was WRONG and must not be reused: that storing display text would leave a permanent Parent/Keeper seam across feed history. False. actor_label stores member names, not role words, and the Keeper and Kin rename touched zero stored rows. UNBLOCKED BY THIS: logActivity moving server-side is no longer blocked on a design question, and it is not even a proposal, because admit_pending_member and deny_pending_member already bypass the client helper and INSERT from inside the SQL function, and they are the only two sites where the label is server-verified against the real identity. The pattern is shipped twice and working. STILL OPEN AND SCOTT'S: whether the resolved name is frozen at write time or looked up live. The schema comment reads "frozen display name" and that is deliberate. That question blocks the backfill design, not the schema shape.
REPLACES: The "What does actor_label mean?" open decision (raised 2026-07-30) and the "does the display label belong in the DB at all?" question that had been blocking the logActivity server-side move.
STATUS: LOCKED

---

DECISION: The audience badge shows only restrictions. Absence of a badge is the indication for the unrestricted case.
DATE: 2026-08-03
WHY: The audience field has three values: anyone, parents-only, kids-only. Under the rename the restricted labels become "Keepers only" and "Kin only". The unrestricted case renders NOTHING, because the restriction is the information and an unrestricted bounty sitting on a Kin's board already tells them it is for them. This also killed the "Available to anyone" line on QuestCard, which was a genuine defect independent of the rename: it never read quest.audience at all, rendering whenever there was no assignee, so a parents-only bounty could display the Keepers only badge and a line claiming it was available to anyone on the same card. Deleted on two independent arguments, and the stronger one was Scott's: it cost a line of vertical height on every unrestricted card on a scrolling phone board, which is the one resource a phone surface cannot spare. Also corrected: board.tsx's "Anyone can claim" kicker and "Open bounties · anyone can claim" heading both sat above lists that INCLUDE restricted bounties for a Keeper viewer. Fixed by removing the claim rather than making it conditional. A jAIne argument that was WRONG: that "Kids only" was always a lie because the flag never excluded adults. The field has three values, kids-only is a real restriction that does exclude Keepers, and the copy defect jAIne claimed to have caught did not exist. The brief had been written from a recon inventory that listed the label without listing the field's value set, and jAIne never asked what the values were. The stop-clause caught it before it shipped.
REPLACES: Nothing — new decision.
STATUS: LOCKED

---

DECISION: The kids-only audience value is suspected dead, and the reason is structural.
DATE: 2026-08-03
WHY: Scott is roughly 92% confident that no bounty in his hold has ever been set to kids-only, and the Slate shows no recurring ones. The rename exposed WHY, and the why is not forgetfulness. "Kids only" read as a rule a Keeper sets. "Kin only" reads as a description of who happens to be around. The tag was never doing anything except borrowing authority from the word "kid", and the rename stripped it out. Structurally: Keepers only excludes a Kin from something they should not have. Kin only excludes a Keeper from something they are perfectly capable of doing, in a household where the Keeper is the one who wants the work done. There is no family scenario where that direction is useful. A restriction pointing up the authority ladder in a chore app has nothing to restrict. The "Kin only" badge shipped anyway on 2026-08-03, because a badge is five lines and removing an enum value is a migration plus a filter rewrite plus a decision about what existing rows become, and 92% is not zero. WHAT SETTLES IT: one grouped read of quests.audience across all thirteen holds, free and read-only, which should ride along on the next Code job rather than justify its own. If it comes back empty, killing the value is a cheap migration on empty data and belongs in the Phase 2 pre-production sweep, taking the badge with it. Do not kill it on 92%.
REPLACES: Nothing — new observation.
STATUS: NOTED

---

DECISION: parent_self_redeem skipping the approval gate is intended behavior, ratified.
DATE: 2026-08-03
WHY: An adult redeeming their own embers inserts status='approved' with decided_by = auth.uid() in one call, making them requester and approver simultaneously. This reads as deliberate in the code and has never been written down. Scott's call, immediate and without hesitation: it is acting as intended. Logged as a ratification rather than a decision, because it was never an open question. Undocumented intended behavior is a doc defect, not a design fork, and it had been sitting in OPEN DECISIONS under a false premise. The rule it appears to violate, that real-world redeemability requires a second party to mint, is about MINTING and about protecting a Kin's earned balance from self-approval. A Keeper spending their own embers on a reward they themselves stocked has no second party to protect and no economy to distort. The gate exists to make approval mean something, and approval means nothing when the approver and the requester are the same authority tier by design.
REPLACES: The "Is parent_self_redeem skipping the approval gate correct?" open decision, raised 2026-08-02. It should never have been filed there.
STATUS: LOCKED
