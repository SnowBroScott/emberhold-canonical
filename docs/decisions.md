...
DECISION: Quest creation is adult-gated. The join path never offers a PIN.
DATE: 2026-07-26
WHY: Recon established that quest creation is gated nowhere — no client check, and the RLS INSERT policy on quests scopes to family_id = current_family_id() and created_by = auth.uid() and stops there, in explicit contrast to the UPDATE and DELETE policies and enforce_quest_update_authority, all of which do check for parent. So any active member could post a quest at any point value. jAIne surfaced this as a NOTED observation leaning toward leaving it alone, on the grounds that a kid proposing a chore is a good thing and feeds the Open Bounties model, and that the economy was never at risk because approval is parent-gated — a kid could propose a 500-ember quest and could not cash it. SCOTT'S CALL: adults only. The proposing-kid argument is real but it is a feature nobody designed, arriving through an unwritten permission rather than a decision, and an undecided permission is one somebody eventually "fixes" in the wrong direction at the worst possible moment. Role-aware depth is a stated discipline: kids see a game, adults see operations. Authoring the work is operations. BUILD SHAPE, AND BOTH HALVES ARE REQUIRED: a has_role(auth.uid(),'parent') clause on the quests INSERT policy, PLUS the client-side gate. A client gate alone is theatre — the whole reason this was found is that the client and the policy disagreed about who is trusted. THE TRAP, NAMED BECAUSE IT WILL BITE: FirstQuestDoorway renders for kids as well as adults, and the onboarding first-quest screen is reachable by both. Gating creation without gating those surfaces walks a kid into a permission wall behind a friendly mascot — the exact "where'd my chore go" failure aimed at the person the app exists to be fun for. The gate and those role conditions ship together or not at all. Also unconditioned today and easy to miss: the FAB in BottomBar.tsx, plus entry points in board.tsx, campaign.$id.tsx and QuickAddTray.tsx. THE JOIN + PIN BUG, FIXED SAME SESSION AND RELATED BY SUBJECT: runSetup called set_profile_pin whenever role === 'parent' && pin, but complete_household_setup always creates a joining member as kid/pending regardless of requested role, so a joining adult who set a PIN triggered "Only parents can set a PIN" server-side and received a generic error toast at the end of signup. That was firing for every joining adult in every household. The join path now renders no PIN field and set_profile_pin fires only when familyChoice === 'create'; a joining member's PIN is set after an existing parent admits them.
REPLACES: Nothing — new decision. Settles a permission that was inherited rather than chosen.
STATUS: LOCKED
