...
DECISION: Quest creation is not gated to adults, and the join path never offers a PIN.
DATE: 2026-07-26
WHY: Two findings from the same recon, recorded together because both concern who is allowed to do what during and after setup. QUEST CREATION IS UNGATED — recon confirmed there is no adult check anywhere, client-side or in policy. The RLS INSERT policy on quests scopes to family_id = current_family_id() and created_by = auth.uid(), and stops there, in explicit contrast to the UPDATE and DELETE policies and enforce_quest_update_authority, all of which do check for parent. So any active member can post a quest at any point value. THE ECONOMY STILL HOLDS: approval is parent-gated, so a kid can propose a 500-ember quest and cannot cash it. This is arguably correct behavior — a kid proposing a chore is a good thing and it feeds the Open Bounties model — but it was inherited rather than decided, and an undecided permission is one somebody eventually "fixes" in the wrong direction. Recorded so the next person to notice it finds a note instead of a surprise. THE JOIN + PIN BUG, FIXED SAME SESSION: runSetup called set_profile_pin whenever role === 'parent' && pin, but complete_household_setup always creates a joining member as kid/pending regardless of their requested role. So a joining adult who set a PIN triggered "Only parents can set a PIN" server-side and got a generic error toast at the end of signup. That was firing for every joining adult in every household. The join path now renders no PIN field and set_profile_pin fires only when familyChoice === 'create'. A joining member's PIN is set after an existing parent admits them.
REPLACES: Nothing — new decision.
STATUS: NOTED
