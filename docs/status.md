...
> **`north-star.md` is still out of date and it is the doc that says "read this first, cold."** It knows nothing about the constitution restructure or the module architecture, and it still owes the success definition and the $636 target. **The protocol blocker is dead** — north-star is now an allowed fifth close-out deliverable. Pick option (a) fold-at-close or (b) dedicated pass.
...
**Off the critical path but the best ROI on the board: the fitness module, Option A.** Replacing Fitbod at $15.99/mo is ~$192/yr against a $636/yr break-even target — roughly a third of break-even recovered with no paywall, no funnel, and no Gate E dependency.
...
- 🟡 **`/setup/intent` has still never been loaded by a human**, and it is still unrouted and unread. **Decide its fate in the same pass that finishes onboarding** — it is the seed of module intent, which was deliberately cut from this build. Third session carrying it.
...
- ⬜ **Quest creation is not adult-gated — anywhere.** No client check, no policy check. Any active member can post a quest at any point value. **The economy still holds** because `enforce_quest_update_authority` gates approval on parent, so nothing mints without an adult — a kid can propose a 500-ember quest and cannot cash it. **Arguably correct** (a kid proposing a chore is good), but it was never decided, just inherited. Decide deliberately.
...
- ⬜ **`north-star.md` is out of date** and it is the doc that says "read this first, cold." It also still owes the success definition and the $636 target. **Protocol blocker is dead; it is now an allowed fifth close-out deliverable.** Pick (a) fold at close or (b) dedicated pass.
