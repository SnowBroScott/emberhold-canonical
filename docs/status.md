...
Last session: **2026-07-19 (morning)** — *wall display-mode fine-tune, shipped and verified on the glass; a monetization riff hardened into a real SKU shape.* Two wall tweaks landed end-to-end on the physical iPad. The "Who's on the Hearth" avatars grew ~18% of tile width — a real, visible step, not a rounding-error step. And the quest-count line's two-line wrap on the wall was killed for good. The wrap fix carried a model correction: `whitespace-nowrap` doesn't survive the flex-item boundary in iPad Safari, so the durable fix was an NBSP between the label words — no breakable whitespace left to break. A second wall-geometry rule fell out of the avatar bump: "make it bigger" is never just the size token — the ring halo and tile padding eat most of a raw increase, so you pay size + ring-inset + padding together or it reads as no change. Both rules logged (decisions.md). Around the build, the theme-pack monetization idea hardened from "palette packs" into **living-hold ambient themes** — four registers as moving scenes, differentiated by emotional register, canvas-particles not bespoke video, with Endure as a proven-but-bespoke reference engine. Parked with receipts. Credits ~4 remaining; Lovable's scanner surfaced 8 security findings → the natural Code-lane interim work while the tank refills.

Last session (prior): **2026-07-18 (evening)** — *avatar transport shipped, and a three-item QA burn-down.* The long-tabled Gate-A roster item came off the shelf and landed end-to-end: 48-avatar roster live, tier-tagged free/founder (16/32), founder gate built and dormant, re-forge run on the glass. Around it: three QA fixes shipped (Ranks per-member completed-quest expand, phone + wall; calendar month view now shows the whole visible month past + future, phone + wall; the "hero" vocab leak in the wall's claim picker killed). One QA item (#5 "hold admin role") reclassified as distribution-era support tooling, not a home-hold fix. One live defect found and fixed twice over: invalidated avatar picks were rendering their raw id as large text (the "ugly number") — a render-fallback ("the floor") is building at close to render a clean placeholder instead. Two real open findings surfaced (re-forge reach across the 13; render fallback pending glass-verify) plus four new P4×L8 audit inputs from the gate's new DB objects.
...
| # | Item | Blocks |
|---|---|---|
| **1** | **P4×L8 — the tenant-isolation audit (Workstream 1).** Opus, auto-accept OFF. The mortal-peril item and the last security gate before distribution. Hold Alpha exists — run it as an attack. Banked inputs: 07-15 live-catalog dump · 25 linter warnings · 07-16 Data-API grant surface · `wall_request_redemption` household-scope · commit-attribution class (`approved_by` client-asserted, `decided_by` session-locked) · the founder-gate DB objects (four inputs below) · **(new 07-19) the 8 findings from Lovable's own scanner — triage against this list first (below).** First: confirm the adversarial harness can reach Lovable Cloud's DB from outside (parking-lot open decision) — if not, the audit degrades to policy-reading. | Distribution. Gate B. |
...
**Next-action fork (Scott's call):** honest sequence remains **P4×L8 first** — it's the gate, everything downstream sits behind the distribution it unblocks. The contained non-Opus win is still **the auth email**. **Credit-dry interim (credits ~4):** the **8 Lovable security findings** are the natural Code-lane job — text-verifiable, zero Lovable credits, useful while the tank refills.
...
- ✅ **"Who's on the Hearth" avatars enlarged.** Wall avatar wrapper `min(66%, 11.5rem)` → `min(78%, 13rem)` (~18% of tile width), ring offset pulled `inset-[-4px]` → `inset-[-2px]`, tile padding tightened (`sm:p-3`→`sm:p-2`, gap `gap-1.5`→`gap-1`) so the added size isn't re-consumed by halo/padding. Avatar now reads as the dominant element of each tile.
- ✅ **Quest-count line wrap killed.** `4 QUESTS OPEN` / `1 QUEST OPEN` were wrapping to two lines on the wall (label breaking across `QUESTS`/`OPEN`, orphaning the numeral). Root cause: `whitespace-nowrap` on the parent `<p>` was lost across the flex-item boundary and iPad Safari broke the internal space. Fix: NBSP (`\u00A0`) between the label words removes the breakable whitespace entirely; `whitespace-nowrap` added on both inner spans; row switched to `flex w-full min-w-0` + `items-baseline`; label clamp lowered for headroom. Now one centered line at the same baseline rhythm as `ALL CLEAR`.
- Scope held: `WallMemberTile` only; no other tile, no other surface, the `Avatar` component untouched.
- **Two durable wall-rendering rules captured** (see decisions.md): NBSP-not-nowrap for wall text that must never break; the halo/padding tax on any wall avatar resize.
...
- ⬜ **8 security findings surfaced by Lovable's own scanner (new 2026-07-19) — the credit-dry interim work.** With Lovable credits at ~4, these are the natural Code-lane job (text-verifiable, zero Lovable credits) while the tank refills. **Un-triaged** — pull the actual list and cross-check against the known P4×L8 inputs and the founder-gate DB objects below before spending Code cycles; some may overlap, some may be new. ⚠️ *Do NOT write exploit specifics into this repo (public) — findings detail lives in the Workstream 1 / Code session.*
...
See `parking-lot.md`. **Living-hold theme packs — monetization SKU #2** (new 07-19) · **QA #5 super-admin / tier-2 support role** (07-18) · **Re-forge reach across the 13** (07-18) · Favorites on the wall · role-label retirement ("Parent/Kid," esp. "Kid") · #8b admin-reporting · kid-vs-kid impersonation · kid-auth (declined) · photo avatars · cosmetic drop #2 (the roster surplus) · Capacitor · flat/peer holds · recurrence chip legibility · the "how Scott & jAIne work" collaboration profile.
