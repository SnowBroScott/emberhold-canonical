...
- ⬜ **🔴 QUEST CREATION MUST BECOME ADULT-GATED — DECIDED 07-26, NOT YET BUILT.** It is currently ungated everywhere: no client check, and the RLS INSERT policy on `quests` scopes to `family_id` and `created_by` and stops there, in explicit contrast to the UPDATE/DELETE policies and `enforce_quest_update_authority`, which do check parent. **Scott's call: adults only.** Build shape: a `has_role(auth.uid(),'parent')` clause on the INSERT policy **plus** the client-side gate, together — a client gate alone is theatre. ⚠️ **THE GATE AND THE DOORWAY MUST SHIP TOGETHER.** `FirstQuestDoorway` renders for kids as well as adults, and the onboarding first-quest screen is reachable by both. Gating creation without gating those surfaces walks a kid into a wall behind the mascot — the exact "where'd my chore go" failure aimed at the person the app exists to be fun for. Also check the FAB in `BottomBar.tsx`, `board.tsx`, `campaign.$id.tsx` and `QuickAddTray.tsx`, none of which are role-conditioned today.
