...
- **Ranks as a household dial** — soften the sibling-ladder into private progress. jAIne has a lean; Scott hasn't called it. *(2026-07-16: Ranks now also appears as a wall-native panel in the display accordion — a second surface for the same data, which may sharpen this call.)*
- **Display mode's position on the ladder** — simultaneously "the unproven core bet" and "fenced to post-launch." Both defensible; they can't both be true forever. **Note (2026-07-15): it's also the one delight item that doesn't need the security gate open** — it's for Scott's own household, not strangers — which makes it the defensible morale pull-forward if a fun session is wanted before P4×L8. **Update (2026-07-16): a working design prototype now exists** (built this session in Lovable — see the fleshed-out LATER entry below). The tension is unchanged, but the "unproven" half is now *less* unproven: the layout works and looks right. See `north-star.md`.
...
- **Vault favorites → real per-profile persistence** — `localStorage` won't survive the shared-wall model, and favoriting is load-bearing. *(2026-07-16: the wall display makes the shared-wall model concrete — this debt now has a surface that will expose it.)*
...
- **Wall calendar — color event pills by member (2026-07-16, Scott's parting note).** The class→color the member tiles glow with should carry onto the month-grid event pills, so "whose event" reads pre-attentively at distance (legibility-at-distance again). Either the whole pill tints to the member's class color, or a small class-colored dot rides inside it. "AJ's bday party" → green (Garden), etc. Map already exists (it's the tile glow). *Scott's call on dot-vs-full-tint; a "see it both ways" when the wall work resumes. Applies to the display prototype only.*
...
- **Display / wall / kiosk mode — DESIGN PROTOTYPED 2026-07-16 (Lovable), not wired, not on the critical path.** The ambient-presence thesis, now a working landscape layout instead of a description. Built as the fenced-delight morale pull-forward; still fenced (it's a retention feature, and retention comes after there's someone to retain — see `north-star.md`). **Design-complete ≠ shipped:** the prototype is read-only — nothing is wired (no claim / turn-in / redeem, no PIN flow, no realtime/polling, no idle timer). Target hardware: an old iPad mini, wall-mounted, landscape ~1024×768 (4:3).

  **The layout — four zones:**
  - **Top banner** (full width, short): hold name + a running activity-feed ticker (recent completions/quest posts with ember amounts). Household + passive.
  - **Left ~2/3:** the calendar — **agenda list by default**, with a small **icon toggle** (list glyph / calendar glyph, not word-pills) to switch to a **month grid**. A slim campaign-progress bar is anchored at the bottom of this column.
  - **Right ~1/3:** the **accordion column** (see below).

  **THE THREE LAWS this session produced** *(currently captured as "might be" — see promotion trigger at the end):*
  1. **The spatial law — household+passive vs. person+active.** Household-level, glanceable, stateless things live in the top banner and left column (you look, you don't act). Person-specific, stateful, "act as someone" things live in the right third, and *only* there. This law is **predictive, not just tidy**: it correctly called that Open Bounties stay glanceable up top (an unclaimed bounty belongs to *nobody* yet → household-scoped) while the **Vault routes through each member's tile** (inherently person-scoped → the tap establishes identity, so no "who's redeeming?" picker is needed). It also **flagged its own violation**: the Ranks button Lovable wired to the full-screen leaderboard *clashed*, because a navigate-away surface is foreign to a swap-in-place wall. The wall has an immune response — that's the law working.
  2. **Legibility-at-distance sizing.** On a wall read from across a room, element size tracks *legibility-at-distance, not aesthetic weight*. A face is legible even when small; text and data are not. So avatars stay modest and the data around them (quest counts, event titles, dates, feed, campaign text) goes large. Corollary lesson, hard-won: "make the month calendar bigger" **failed twice** because cell size was never the problem — the fix was making the **event MARKER** loud (a filled ember-colored chip with readable text, not bottom-anchored ghost text). Fix what you're actually aiming at.
  3. **The accordion column.** The right third is a 3-zone accordion — one interaction primitive governing all three: **Open Bounties** (top), **Who's on the Hearth** (middle), **Ranks** (bottom). Tap any collapsed header → it takes the column body; the other two shrink to header bars (one pinned top, one pinned bottom), each tappable to swap back. The zones take turns owning the space; none ever overflows into the left column or banner. **Ranks renders as a wall-native panel** (reuse the leaderboard *data*, ember-lit, matching the bounties-list styling — do NOT navigate to the full-screen leaderboard).

  **The resting state (now concretely defined):** Bounties collapsed (top) · **Hearth EXPANDED** (middle — this is "home") · Ranks collapsed (bottom) · **agenda** (not month) on the left. "Ambient" is no longer vague — it's *this named configuration*.

  **Idle-return-to-ambient (the missing primitive, NOT built yet):** the wall always drifts home. Any deviation — someone opened Ranks, toggled to month, expanded Bounties — decays back to the resting config above after a quiet idle period. This is what makes a wall display a *display* and not a tablet someone left on a screen. The accordion made "home" concrete, which makes this buildable. *Not built this session (kept the fun-session bounded); it's the next real interaction primitive.*

  **The propose/commit PIN model (walk-up on a wall):**
  - **Visibility: fully open.** Tap anyone, see their quests. Zero risk, pure ambient value.
  - **Propose actions** (claim a bounty, turn in a quest, request a redemption): unguarded — or behind a *single shared HOUSEHOLD display PIN* if queue-noise (a toddler mashing, a guest) becomes a problem. **NOT a per-person PIN** — kids have no PINs (kid-auth DECLINED 2026-07-10); actor identity stays cosmetic, "you're one of us" is the only check.
  - **Commit actions** (approve a completion → mint; approve a redemption → spend): **UNCHANGED, sacred, server-side real adult PIN.** This is the existing commit-gate; the wall neither adds nor weakens it. Parental accountability is *structural* via the approval queue, not friction bolted onto the display.
  - This is the **2026-07-15 walk-up trust boundary applied to a wall** — consistent, not a new security model. **Feeds the P4×L8 pile** as an input ("what can an unauthenticated walk-up do on the wall") — but does NOT block the fun build, because the wall is for Scott's own hold, not strangers.

  **Reuse:** ~90% reuse — feed spine, quest tiles, bounties, leaderboard data, calendar/agenda all exist. The genuinely new surfaces are the landscape *arrangement*, the accordion, and (later) polling + the idle timer.

  **Still needs, before it's real (post-fence):** realtime — **polling on an interval, per the PendingWaitingScreen 8s pattern; do NOT build the realtime subsystem for this** · the idle timer · all interaction wiring (claim/turn-in/redeem/PIN) · the real hand-cut roster avatars (the tiles use whatever's live).

  **Deployment gremlins (LATER, don't solve now):** old iPad mini — Safari version, PWA display-mode behavior, screen-sleep (Guided Access or never-sleep + a charging dock), 4:3 landscape ~1024×768. Real, deferred.

  **PROMOTION TRIGGER:** the three laws + the resting-state definition are captured here as "might be." When display mode graduates from fenced-delight to roadmap — post-beta, or the moment Gate E comes back flat on day-8 retention (per north-star, display mode is first off the fence) — **promote the three laws to `decisions.md` / `master-spec.md`.** Not before; locking a fenced feature's internal laws is premature.
...
- **Wall-display design session** — see the fleshed-out LATER entry. Prototyped, not wired; nothing shipped. Captured, not committed.
