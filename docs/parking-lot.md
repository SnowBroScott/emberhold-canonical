...
- **🔴 THE JOINER FLOW HAS NO ONBOARDING — what does a non-creator's first run look like? (NEW 07-26.)** `auth.tsx` routes `create` → the five-screen flow and `join` → `/board` directly. **Every second adult and every kid in every stranger household arrives cold on a board with no guidance.** It is arguably higher-value than the creator flow, because a kid's first run is where day-8 retention actually lives — and the creator flow's screens are all about *setting things up*, which a joiner does not do. **This is a different flow, not the same one re-routed.** A joiner needs: what a quest is, how to claim one, what embers buy, and what the PIN is for. Unowned and unscoped.
- **🔴 What should "Replay the intro" do? (NEW 07-26.)** `PipHelp` already has the button, and it currently opens a four-step sheet. The five-screen setup flow now exists and an established hold *can* reach `/onboarding/first-quest` by URL and be walked through cold-start copy. **Not a bug — the correct behavior depends on what replaying the intro is supposed to mean.** Options: (a) leave the sheet as-is and never point it at the flow, (b) point it at a read-only version of the flow with the writes disabled, (c) let it re-run the real flow and accept that it posts another quest. **Decide before anyone wires it.**
- **🔴 Is quest creation supposed to be adult-gated? (NEW 07-26.)** It is not gated anywhere — no client check, no RLS check. Any active member can post a quest at any point value. **The economy is not at risk** (`enforce_quest_update_authority` gates approval on parent, so a kid can propose a 500-ember quest and cannot cash it). **Arguably correct** — a kid proposing a chore is a good thing, and it feeds the Open Bounties model. But it was inherited, never decided. **Decide deliberately, then either write it down or gate it.**
- **🔴 `/setup/intent`'s fate — third session carrying it.** Unrouted, unlinked, never loaded by a human, and nothing reads `enabled_modules`. **Module intent was deliberately cut from the 07-26 onboarding build** on the grounds that fitness does not exist and a first-run screen offering a module that opens onto nothing is worse than not asking. That reasoning holds — but the screen and the column now sit unowned through a build that touched everything around them. **Either wire it when fitness ships, or delete it and rebuild when there is something to enable.**
- **🔴 THE FITNESS MODULE'S SHAPE — Option A vs Option B.** **A ships first; this decision is only about whether B ever happens.**
  - **(A) TOOL ONLY.** Set logging, automatic programming, rest timing, progressive adjustment, the reason string. No game, no currency, no collection. **Buildable now. Cancels a $15.99/mo subscription this month.**
...
- **🔴 `north-star.md` fold — asked four times, and the protocol blocker is now dead.** It is the doc that says *"read this first, cold,"* and it describes a product that no longer exists: no constitution, no module architecture, no awareness that two of its own stated principles were rewritten. **It also still owes the success definition and the $636 target** — the gate ladder states a sequence and no destination. **north-star is now an allowed fifth close-out deliverable.** Two options: (a) fold it at close, (b) a dedicated pass. **Pick one.**
...
- **The joiner flow.** See OPEN DECISIONS. Different content, same shell — `SetupShell` and the step registry already exist and cost nothing to reuse.
