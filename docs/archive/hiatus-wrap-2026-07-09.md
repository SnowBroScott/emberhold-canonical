...
19. **Join-code security.** No rotation, no join-approval currently exists. If a code leaks or gets brute-forced, the only recourse today is manually deleting the joined member. Needs at minimum: regenerate-code (invalidates the old one) and likely admit-on-approval (an adult confirms a join request before it completes). *(Phaeaz — his sharpest catch.)*
