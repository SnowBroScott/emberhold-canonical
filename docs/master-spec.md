...
- **⚠️ `FirstRunGate` currently reads the ACCOUNT HOLDER, not the switched-into profile.** On a shared device a kid switching in is checked against the account holder's marker and role. **Fix before the column lands or the kid flow never runs for the people it was built for.**
- **Open and scope-determining:** whether a member can UPDATE their own `profiles` row under current RLS. If not, this is a column *plus* an RPC.
...
- **Deadline, and it is not arbitrary:** the string is already live but the flow is dark, so nobody has been promised anything yet. **It must be true before the arrival gate goes live** — which makes it the same change as the completion-marker column, not a separate item.
