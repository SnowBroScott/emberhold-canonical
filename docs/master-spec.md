...
- **Confirmed finding — quest self-approval exposure (PRE-DISTRIBUTION BLOCKER).** The `Update family quests` RLS UPDATE policy gates by *row* with **no column-level restriction** — any claimer/assignee can write `status='approved'`, `approved_by`, and points via the API. Practical risk today ≈ zero (no kid-facing approve path, no kid session), but the rule is unenforced at the policy layer.
  - **The naive fix does not work.** `has_role(auth.uid(), 'parent')` is **insufficient**, because kids act under the owner's authenticated session — `auth.uid()` is *always* the parent. The real fix keys on the **acting profile** + a verified-parent action, and **restricts columns** (kids: claimed/in_progress only; approved / approved_by / points: verified parent only).
  - This is a **Claude Code job** (read the parent-verification path end to end), not a Lovable quick-patch.
- **SECURITY DEFINER audit.** 22 definer functions; 15 trip lint 0029. Sweep the sensitive-signature ones for internal caller-gating: `get_member_email`, `set_profile_pin`, `verify_profile_pin`, `member_spendable`, `approve_redemption`, `deny_redemption`, `parent_self_redeem`, `create_adult_profile`. *(`roll_missed_dailies` anon grant already revoked.)*
