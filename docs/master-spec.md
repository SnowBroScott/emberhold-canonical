...
Last substantive update: **2026-08-01**. **A four-region fold, from a read.** The `verbLabel` open question is answered and closed without a fix; the completion marker's read/write pair is shipped and verified on the glass; `/quest-log` and `/hearth-log` are reclassified from deletion-pending to a deliberate debug surface kept until Gate C; and the copy-discipline section gains the em-dash rule. **One new prohibition, no new architecture.**

Prior substantive update: **2026-07-31** — **a correction pass on the Slate/Ledger region.** Seven lines described decisions as open, undecided or unbuilt that had been decided and shipped by 07-30 and 07-31: the Retired column, the two-brightness warning, the Slate's empty state, the Ledger's shape, the navigation answer, the title-shrink mechanism, and the wall's write posture. **No new design truth — this pass only removed lines that had outlived the build.** *(The 07-30 fold that introduced the Bounty rename, the Slate and the Ledger stands as written.)*

Prior substantive update: **2026-07-30 (late)** — **the object noun changed.** "Quest" is retired; **"Bounty" is the universal object term**, and the old proper-noun carve-out is gone. Quest Log is replaced by two surfaces, **the Slate** and **the Ledger**. The recurring lifecycle gains a decided roll-forward rule. Also folded: user-authored content is out of scope for vocabulary audits, and a design-system rule that the title is never the element permitted to shrink.

> **✅ THE RENAME IS LANDED ON THE GLASS.** *(Sweep 2026-07-30; coverage grep 2026-07-31.)* The grep ran the full working tree and found `onboarding.first-quest.tsx` **completely unswept** — eight user-facing strings including an h1, a form label and a button — plus two single misses elsewhere. All fixed. `public/`, the PWA manifest and every shipped `.json`/`.md` were already clean. **`status.md` owns the landing check and it is closed.** ✅ **THE ONE RENDERING RESIDUE IS SCOPED AND CLOSED WITHOUT A FIX.** *(Recon 2026-08-01, read from source, not inferred.)* `hearth-log.tsx`'s `verbLabel()` special-cases only `bounty_posted` and falls through to `verb.replace("_"," ")`, so the enum leaks onto the glass **there** as **QUEST APPROVED**. **The open question is answered: the other three surfaces are clean.** `wall.tsx`'s `tickerLine`, `Briefing.tsx`'s `pulseLine` and `NotificationBell.tsx`'s `lineFor` are a word-for-word identical switch over five named verbs with a `default` that returns `${who} · ${obj}` and **never touches the raw verb**. **That is duplication, not divergence, and it does not carry the `isActiveQuest` severity.** ⚠️ **CORRECTED 2026-08-01 (late): "word-for-word identical" WAS OVERSTATED, AND THIS DOC CARRIED THE OVERSTATEMENT FOR ONE SESSION.** The five verbs behave identically, which is what the recon was briefed to check. The switches as a whole do not: `wall.tsx`'s `event_created` reads `${actor} added ${obj}` while `Briefing.tsx` and `NotificationBell.tsx` read `${actor} added ${obj} to the calendar`. **Three copies that have already drifted once is a different claim from three copies that have not.** Still duplication rather than a correctness defect, and still not the `isActiveQuest` pattern, but the drift is real and the shorter string is on the wall, which is the semi-public surface. **A recon answers the question it was briefed with. Do not generalize its finding to the container the question lived in.** `hearth-log.tsx` is the sole place in the codebase deriving display text from the raw enum; deleting `verbLabel()` would kill `QUEST APPROVED` outright. ⚠️ **The `activity_verb` enum has SEVEN values and every renderer names five.** `member_admitted` and `member_denied` are written server-side and fall to the label-only default, rendering as `"Mom · Leo"`. **Two missing cases in three near-identical switches. Cheaper than the polish list assumed, and not a defect in the pattern.** ✅ **SHIPPED 2026-08-01 (late), `b2efd8f`.** All three switches gained both cases: `${who} admitted ${obj}` and `${who} denied ${obj}`, matching the existing past-tense shape. `hearth-log.tsx` was deliberately excluded as out of scope. ⚠️ **`member_denied` has never rendered in production**, because nobody has been denied.
...
- **✅ THE MARKER IS PROFILE-SCOPED IN BOTH DIRECTIONS. SHIPPED AND VERIFIED 2026-08-01.** The read and the write were both account-holder-scoped and were fixed as **one change**, which was correct and not optional: **the two bugs cancelled, and fixing the read alone produces the hostage loop.** A kid sub-profile's row id is **not** any `auth.uid()`, so a kid's write had been updating **zero rows and returning success** since the marker was written.
  - **The write:** the zero-argument `mark_first_run_complete()` was **DROPPED, not defaulted**, so no old signature survives as a callable overload. The replacement is `mark_first_run_complete(_profile_id uuid)`, SECURITY DEFINER, `search_path = public`, validating signed-in **plus** `current_family_id()` match **plus** `status = 'active'`, mirroring `wall_request_redemption`. **It makes no comparison against `auth.uid()`. That comparison was the bug.**
  - **⚠️ THE SHIM WAS EATING THE ID.** `markFirstRunComplete(profileId)` contained a literal `void profileId;` and called the RPC with no arguments. **Five call sites had been correct all along.** The defect was one line below every one of them.
  - **The divergence was THREE-WAY, not two-way.** A second reader was owner-keyed too: `FirstRunGate` read the route-context profile fetched `WHERE id = data.user.id`. The real split was **write-owner / read-owner / read-active**, not the two-way split this doc described.
  - **Both readers now call ONE shared `resolveFirstRunProfileId()`** in `src/lib/first-run.ts` (`getActiveMemberIdSync() ?? auth user id`). `redirectIfFirstRunDone()` and `FirstRunGate` **cannot drift, because there is only one expression** rather than the same expression written twice in two files.
  - **VERIFIED ON THE GLASS BY AN ACTUAL PROFILE SWITCH:** adult session, switch to a kid, first run fired, board with KID tag and a live bounty, switch away, switch back, straight to the board. **The second entry is the test. The first always looks fine.**
  - **⚠️ NEW BEHAVIOR, ACCEPTED:** `FirstRunGate` now re-resolves on member switch and reads the resolved profile's role. Correct, and no misfire observed.
  - **This was NOT the `isActiveQuest` divergence pattern.** **Divergence announces itself; cancellation hides.**
...
**RETIRED DUTIES STAY VISIBLE**, dimmed and collapsed. **This is not sentiment.** If archiving a duty makes it vanish, someone re-creates it in six weeks and we are back at the duplication complaint that started this. **The Slate has one job and "does this already exist" includes the ones you turned off.**

- **⚠️ RETIRE WRITES `quests.retired_at`, A NULLABLE TIMESTAMPTZ. IT DOES NOT REUSE `archived`.** *(LOCKED 2026-07-30. This doc previously claimed Retire cost nothing because `archived` already existed — that was wrong and it is recorded here so it is not re-derived.)* `roll_missed_dailies` has been writing `archived = true` for stale daily instances since it shipped, so a Retired section filtered on `archived` would render **every row the engine has ever swept** as a duty a human deliberately retired. Two meanings, one column. **Rejected:** a backfill separating engine-archived from human-archived (requires judgment about historical rows nobody can reconstruct) and a status enum value (touches lifecycle machinery for a display concern). One nullable column, no backfill, historical residue stays invisible — **and it records WHEN, which `archived` never did.**
- **RETIRE REPLACES DELETE on any bounty that has ever been approved. Delete survives only on never-approved bounties.** *(LOCKED 2026-07-30.)* Under roll-forward the two operations genuinely diverge: Delete ends the duty and erases it; Retire ends it and keeps it visible so nobody recreates it in six weeks. Deleting an approved bounty also destroys minted-ember history. Delete stays on never-approved rows because there is nothing to preserve and without it a mistyped bounty is immortal.
- ✅ **Verified on the glass 2026-07-31:** Standing went 5→4, a `Retired · 1` section appeared where none had existed, the row read **"Retired Jul 31"**, and Delete was gone from the panel. **The section was empty before the test, which is what proves the filter is on `retired_at` and not `archived`.**

**BRIGHTNESS IS THE STATE DIAL, REUSED NOT INVENTED** (constitutional rule 6). Bright = demands you now; dim = handled.
- **The health read is visual before it is numeric** — an "X of Y current" counter is a derived nicety, not the mechanism.
- **⚠️ A BRIGHT SLATE IS NOT A FAILURE STATE, AND THIS DOC PREVIOUSLY IMPLIED IT WAS.** *(Corrected 2026-07-31.)* The original line read *"a dim Slate is a well-kept hold,"* which framed brightness as a health score. **The Slate's job is an accurate count of what is live; whether that count is comfortable is not its problem.** On the first lived-in render it opened at `13 need doing` — every stale weekly moved to the current anchor by roll-forward, one row per duty, no pile — and Scott's call after living with it is that **it reads as honest.** If a Monday ever spikes it past comfortable, the lever is **grouping or a softer due-now treatment, never re-hiding work**, which is the duplication complaint that created the surface.
- **⚠️ BRIGHTNESS ON THE SLATE MEANS STATE AND NOTHING ELSE. THE EMBER TIER RAMP DOES NOT DRIVE ROW HEAT HERE.** *(LOCKED 2026-07-30, superseding this doc's own warning that two brightness signals coexist on one row.)* The board can carry two signals because **the ember number IS the point there**; the Slate's entire read is *"is this hold well-kept,"* and a dial that means two things cannot answer one question. The ember value still renders on the row — it must, because it is editable there — it simply stops setting the glow. **The superseded warning told future sessions not to "fix" the double signal, and that was the tell: a signal needing a permanent guard against its own obvious reading is the wrong signal.** The Ledger inherits this for free — everything in it is finished, so nothing demands anything, and a uniformly flat Ledger is the honest render rather than a missing feature.
...
**THE EMPTY STATE IS THE PRODUCT'S ONLY LESSON IN RECURRENCE.** *(LOCKED 2026-07-30, shipped; the CTA verified on the glass 2026-07-31.)* A brand-new hold has no standing duties, so this surface answers *nothing does* — which reads as a dead end unless it is doing another job. **Nothing anywhere else in Emberhold teaches that a bounty can repeat.** Pip's line names concrete duties (the trash, the dishes, Monday laundry) and states the mechanic plainly: it appears here once and stays. Paired with a create affordance at `/create?recurring=true` that **opens with recurrence already on**, so the lesson ends in the thing it taught. **Rejected:** a generic "nothing here yet" empty state, which spends a real onboarding moment on nothing.
...
**ITS SHAPE IS DECIDED AND BUILT.** *(LOCKED 2026-07-30. This doc previously said the shape was undecided and instructed readers not to build from this section — both lines are superseded.)*
- **One list, role-filtered, read-only.** Reverse-chronological with day headers. Member filter defaulting to **self for kids, the hold for adults**.
- **⚠️ THE RECORD-VS-SCRAPBOOK FORK WAS FALSE AND DISSOLVED RATHER THAN BEING DECIDED.** The difference between an audit line and a trophy is **who is reading**, not what the data is — *"Cade · Take out the trash · Jul 28 · +15"* is both. **Role-aware depth is one of the three disciplines: same data, different lens, filters never separate systems.**
- **SOURCED FROM `quests WHERE status='approved'`, NOT `activity_log`.** The intuitive choice is the wrong one: `activity_log` carries known verb drift, the `member_admitted` two-switch bug, and an unresolved question about what `actor_label` means. The `quests` table gives title, embers, member and timestamp clean.
- **READ-ONLY IS A DELIBERATE CAPABILITY REMOVAL** from Quest Log, which allowed editing embers on an approved bounty. **Editing history on a surface that governs a currency is incoherent** whether or not it retro-mints.
- **NO HEAT.** Everything here is finished, so nothing demands anything.
- *Deferred, not decided: grouping beyond day headers.*
...
**✅ THE REDEMPTION REQUEST PATH. ONE INSERT, TWO CALLERS, FIXED 2026-07-31.**
There are **three** write paths into `redemptions` and they are not interchangeable — read this before touching any of them.
- **`wall_request_redemption` (SECURITY DEFINER) is the ONLY request path for a kid, from ANY surface.** It validates that `_profile_id` belongs to the hold and is `active`, derives `family_id`, reward name and cost **server-side**, and inserts `status='pending'`. **As of 07-31 the Vault page calls it too.**
- **`parent_self_redeem` (SECURITY DEFINER) is the adult's own-store path.** It inserts `status='approved'`, `decided_by = auth.uid()` **in the same call** — an adult redeeming their own embers is requester and approver at once. ⚠️ **This SKIPS the approval gate entirely, it reads as deliberate, and until 2026-07-31 it was written down nowhere. Ratify it or change it, but it stops being undocumented behavior of the economy.**
- **A direct client `.insert()` into `redemptions` is DEAD and must not come back.** *(Deleted 2026-07-31.)*

**⚠️ WHY THE DIRECT INSERT COULD NEVER WORK FOR A KID, AND WHY NOBODY NOTICED FOR WEEKS.** The table's INSERT policy has required `requested_by = auth.uid()` since 2026-06-28. **In the shared-session model `auth.uid()` is ALWAYS the hold owner**, regardless of which profile is switched to client-side, while the insert set `requested_by` to the *active member's profile id*. Those two values can only match for the owner, so **the Vault's Redeem button had failed since the day it was written** — and every prior redemption test had been run as an adult. **The path was never regressed; it was never exercised.** The wall's identical-looking redemption worked only because its RPC is `SECURITY DEFINER` and bypasses table RLS entirely.
- **Rejected — tightening `wall_request_redemption` to enforce the policy's self-attribution.** That forces `_profile_id = auth.uid()` and means **only the owner could ever redeem from the wall.** The recon that recommended it treated the older policy as correct and the newer RPC as deviant; **it is the other way round — the policy compares a profile id against an auth id, an assumption the sub-profile model outgrew.**
- **Rejected — changing the RLS policy.** A migration, and it widens a security surface to fix a frontend routing mistake.
- **THE GENERAL LESSON, AND IT IS THE SAME CLASS AS THE FIRST-RUN MARKER:** ⚠️ **AN ADULT PROFILE ID IS NOT ALWAYS A USER ID, AND A PROFILE ID IS NEVER AN AUTH ID.** Second confirmed instance. **Any policy, trigger or RPC comparing an id against `auth.uid()` must state which kind of id it holds.**
...
**⚠️ `logActivity` IS A CLIENT-SIDE CALL BOLTED ONTO EACH CALL SITE BY HAND, AND ONE SITE FORGOT.** *(Found and patched 2026-07-31.)* The wall's approve mutation called the shared `approve_redemption` RPC and **never logged**, while `vault.tsx`'s always did — so **every redemption ever approved from the wall was invisible to the feed, the ticker and the Hearth Log.** Embers moved and nothing recorded it. The call was not failing or conditional; it was **absent**, and had been since the wall shipped. **The instance is fixed. The mechanism is not:** every new call site can forget the same way. The mechanism fix is the definer RPC writing its own log row — a migration, and **not obviously right**, because four surfaces already render their own display lines from the verb enum and the label may not belong in the database at all. **Open decision, tracked in `parking-lot.md`.**
...
- **⚠️ THE WALL IS A FULL PARTICIPANT IN THE ECONOMY, NOT A DISPLAY.** *(Corrected 2026-07-31. This line previously read "IT NEVER MINTS, SPENDS, APPROVES, OR EDITS" and then contradicted itself in its own next clause. The absolute was false for weeks of shipped behavior and it cost a session's worth of misdiagnosis — a working wall redemption was reported to Scott as a membrane breach.)* The wall **mints** (turn-in approved behind an adult PIN), **spends** (redemption request), and **approves** (PIN-gated, and it is the only surface where approval happens away from a personal device).
- **WHAT THE WALL DOES NOT DO is become a second game.** No separate economy, no separate rules, and **nothing happens there that could not happen on a phone.** That is the membrane, correctly stated. **`wall_request_redemption` inserts `status='pending'` and never touches `decided_by`** — the request and the approval remain two acts by two parties, exactly as they are everywhere else.
- ⚠️ **`wall_request_redemption` IS ALSO CALLED BY THE VAULT AS OF 2026-07-31 AND ITS NAME NOW LIES.** Deliberate debt — renaming an RPC is a migration. **See the redemption-request entry in the Vault section.**
...
- **✅ THE SLATE/LEDGER HALF IS ANSWERED AND SHIPPED. NEITHER GETS A TAB.** *(LOCKED 2026-07-30.)* The question resolved cheaper than feared once the entry point was actually looked at: **Quest Log lived in a Briefing card labelled ARCHIVE** — the surface had already been demoted to a junk drawer by its own label. **The Slate replaces that card; the Ledger is reached by a secondary link from the Slate.** The tab bar stays at seven. *"Does this already exist"* is a **creation-time** question, not a daily-driver destination. **Rejected:** a symmetric segment control (jAIne's earlier lean), which implies equal weight between the two and would have lied about how often the Ledger gets opened.
- **The Hearth Log was deleted outright, not inherited by the Ledger.** It was raw `activity_log` rows behind a card reading VERIFICATION · TEMPORARY — a debug window shipped to production. Letting the Ledger absorb its job builds the admin/reporting surface, which north-star fences explicitly. **Two cards became two cards by coincidence of position, not function.**
- ⚠️ **`/quest-log` AND `/hearth-log` REMAIN ROUTABLE AND UNLINKED, AND DELETION IS NOT A FOLLOW-UP.** *(Reclassified 2026-08-01, superseding "route deletion is a follow-up, after the Slate is trusted.")* They are a **deliberate debug surface with a live consumer: Scott.** Their copy stays deliberately unswept so it remains obvious which surface is which, and **`QUEST APPROVED` rendering on `/hearth-log` is ACCEPTED, NOT OPEN.** See the `verbLabel` note at the top of this doc. **They are removed at Gate C, before production, not before.** ⚠️ **Name every consumer of a route before removing it, and ask whether YOU are one.**
...
- **⚠️ THE TITLE IS NEVER THE ELEMENT PERMITTED TO SHRINK.** *(LOCKED — direction settled 2026-07-30; the mechanism was resolved by recon and by the Slate build, and the scope SHRANK. See the re-scope note below.)* Reported as a wall problem and corrected in the same breath to **wherever bounty strings run long**: Vault manage rows, the wall's Claimable list, the wall's Recently Completed list. In every case the ember value, timestamp and action icons are shrink-protected and **the title absorbs the entire squeeze.** Rows are **vertically generous and horizontally starved.**
  - **⚠️ THE SHARED ROW PRIMITIVE IS SUPERSEDED. THE JOB IS SMALLER THAN THIS DOC CLAIMED.** *(2026-07-30 night.)* The prescription was a primitive threaded through ~18 call sites. **Two independently-authored surfaces — the board's `QuestCard` and the Slate — both wrap correctly with no primitive at all.** *Not a single title truncated on the Slate on first render; "Change sheets in Master Bedroom" wrapped to two lines in the densest row in the product.* **The defect is three call sites that declared `truncate` explicitly, not an absent shared component.** The job is **deleting a class at three sites.**
  - **⚠️ THE WALL IS THE OPEN HALF AND IT IS SCOTT'S EYEBALL, NOT A BRIEF.** It is a fixed-height ambient rail, so wrapping there may push rows out of view where wrapping on a scrollable phone surface costs nothing. **Cap-at-two-lines may earn its keep on the wall and only on the wall.**
  - **THE MECHANISM WAS FOUND AND IT IS NOT WHAT WAS SUSPECTED:** `truncate` is applied **unconditionally**, and `overflow-hidden` zeroes the title's flex auto-min-width, so the title absorbs the squeeze **by CSS default rather than by anyone's declaration.**
...
- **⚠️ TWO CANON DOCS CAN CONTRADICT EACH OTHER AND AN AGENT WILL READ BOTH.** *(2026-07-31.)* `north-star.md` said the wall never spends; `status.md` listed wall-approve-with-PIN as a pending verify. Both were fetched at the same session open. jAIne reasoned off the **principle** rather than the **shipped behavior** and reported a working feature as a membrane breach. **The doc describing what the system DOES outranks the doc describing what it SHOULD DO** — the first is falsifiable against the glass, the second is an intention that may have been superseded and never written down. **And the contradiction is itself a finding: an agent that silently picks the right doc has fixed its own turn and left the trap armed for the next instance.**
- **⚠️ NEVER-WORKED AND BROKE LOOK IDENTICAL FROM THE GLASS.** *(2026-07-31.)* The Vault's kid redemption was diagnosed as a regression and the most recent migration was the first suspect. It had never worked once. **Ask whether anyone has ever exercised a path before asking what changed** — and *"verified"* must name **who walked it**, because that button worked for every adult who ever pressed it.
- **⚠️ A RECON'S FORENSICS AND ITS RECOMMENDATION ARE SEPARATE ARTIFACTS.** *(2026-07-31.)* A recon can be flawless on mechanism and wrong on the fix — this one's recommendation would have locked every kid out of wall redemption. **Read the evidence, discard the conclusion, decide separately.**
- **⚠️ A TEXTUAL SWEEP CANNOT FIND A BUG THAT IS NOT A STRING.** *(2026-07-31.)* `QUEST APPROVED` renders from `verb.replace("_"," ")`. No line in the repo spells it. **Grep has a structural blind spot: anything the code assembles at runtime.**
- **⚠️ DESIGN-RATIONALE PHRASING ESCAPES FROM THE BRIEF ONTO THE GLASS.** *(2026-07-31.)* *"Everything with a future,"* *"recurring duties — one line, forever,"* *"here until they're approved"* were jAIne's own reasoning, written into a build prompt as the WHY and rendered by Lovable as UI. **The app ended up explaining its own architecture to a nine-year-old.** The rule: **delete copy that explains something the screen already shows; reword only what teaches something invisible.** Counter-examples that stay: the Vault's *"Each request goes to an adult"* and the wall's *"AN ADULT APPROVES ON THEIR PHONE. NO EMBERS MOVE YET."* — both teach the approval gate, which is invisible and is what people get wrong.
- **⚠️ JAINE'S PUNCTUATION REACHES THE GLASS THE SAME WAY HER RATIONALE DOES.** *(2026-08-01.)* **125 em-dash characters across 43 files** shipped to users, including page titles, `Skip` labels, PIN ranges and roughly forty individual lines, because jAIne writes em dashes into build prompts and Lovable renders the phrasing verbatim. **Same disease as the escaped subtitles, wearing a different tell.** **The rule: no em dash in anything jAIne writes, including briefs handed to Code.** The brief is not a private channel; it is a draft of the product's voice.
  - **⚠️ THE EN DASH IN A TIME OR DATE RANGE IS CORRECT TYPOGRAPHIC CONVENTION AND IS NEVER THE TARGET.** Nor is a bare `—` used as a placeholder glyph for "nothing set." A sweep that touches either has overreached.
  - **A FLAGGED LINE IS A BETTER OUTCOME THAN A GUESSED ONE.** The sweep that produced this rule was instructed to flag rather than guess and abstained on seven lines. **All seven abstentions were correct.**
