...
> **✅ THE RENAME IS LANDED ON THE GLASS.** *(Sweep 2026-07-30; coverage grep 2026-07-31.)* The grep ran the full working tree and found `onboarding.first-quest.tsx` **completely unswept** — eight user-facing strings including an h1, a form label and a button — plus two single misses elsewhere. All fixed. `public/`, the PWA manifest and every shipped `.json`/`.md` were already clean. **`status.md` owns the landing check and it is closed.** ⚠️ **One residue is a rendering bug rather than a string and grep cannot find it:** `hearth-log.tsx`'s `verbLabel()` special-cases only `bounty_posted` and falls through to `verb.replace("_"," ")` for every other verb, so the enum leaks onto the glass as **QUEST APPROVED**. That file is the protected rollback path and is slated for deletion; **the open question is whether the same fallback exists in `wall.tsx`, `Briefing.tsx` and `NotificationBell.tsx`.**
...
- ⚠️ **`/quest-log` and `/hearth-log` remain ROUTABLE AND UNLINKED as the rollback path**, and their own copy is deliberately unswept so it stays obvious which surface is which. **Route deletion is a follow-up, after the Slate is trusted.**
